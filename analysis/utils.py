...
def ensure_binary(series: pd.Series) -> pd.Series:
    """Coerce a series into {0,1} float; preserves NaN."""
...
def load_frames(
    clean_path: str | os.PathLike = DEFAULT_CLEAN,
...
def numeric_features(df: pd.DataFrame, exclude: set[str] | None = None) -> list[str]:
    exclude = set(exclude or [])
    return [c for c in df.columns if c not in exclude and pd.api.types.is_numeric_dtype(df[c])]

def categorical_features(df: pd.DataFrame, exclude: set[str] | None = None) -> list[str]:
    exclude = set(exclude or [])
...
def cohen_d(x: pd.Series, y_bin: pd.Series, min_n: int = MIN_N):
    a = x[y_bin == 1].dropna()
    b = x[y_bin == 0].dropna()
    n1, n0 = len(a), len(b)
    ...
    if s_pooled == 0 or np.isnan(s_pooled):
        return None
    return dict(d=float((m1 - m0) / s_pooled), n1=int(n1), n0=int(n0), mean1=float(m1), mean0=float(m0))
...
def quantile_buckets(s: pd.Series, q=(0, 0.25, 0.5, 0.75, 1.0)):
    non_na = s.dropna()
    if non_na.empty:
...
def score_numeric(abs_d: float, n1: int, n0: int) -> float:
    return float(abs_d) * float(np.sqrt(n1 + n0))

def score_rr(abs_log_rr: float, n_level: int, n_rest: int) -> float:
    return float(abs_log_rr) * float(np.sqrt(min(n_level, n_rest)))

def outcome_set(outcomes: str | Iterable[str] | None) -> list[str]:
    if outcomes is None:
        return ["graduated_me"]
    if isinstance(outcomes, str):
        parts = [p.strip() for p in outcomes.split(",") if p.strip()]
        return parts or ["graduated_me"]
