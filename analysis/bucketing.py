...
from .utils import load_frames, numeric_features, quantile_buckets, ensure_binary

def bucket_summary(df: pd.DataFrame, outcome_col: str, cols: list[str] | None = None, q=(0, .25, .5, .75, 1.0)) -> pd.DataFrame:
    ...
    for c in cols:
        if c == outcome_col:
            continue
        b, edges = quantile_buckets(pd.to_numeric(df[c], errors="coerce"), q=q)
        if b.isna().all():
            continue
        tmp = pd.DataFrame({"bucket": b, "y": y}).dropna(subset=["bucket"])
        tab = tmp.groupby("bucket", observed=True, sort=False)["y"].agg(["count", "mean"]).reset_index()
